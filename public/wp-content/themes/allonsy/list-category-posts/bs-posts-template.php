...
while ( have_posts() ):
  ...
  // Article Left
  $lcp_display_output .= '<div class="article-top">';

    //Post Thumbnail
    $lcp_display_output .= $this->get_thumbnail($post);

  $lcp_display_output .= '</div>'; // clost Article Left

  // Article Right
  $lcp_display_output .= '<div class="article-bottom">';

  //Show the title and link to the post:
  $lcp_display_output .= $this->get_post_title($post, 'h4', 'lcp_post');

  // Begin Post Meta - Date, Category, Author, Comments
  $lcp_display_output .= '<p class="post-meta">';

    // Show date:
    $lcp_display_output .= ' ' . $this->get_date($post, 'span', 'post-meta-date');
    ...
    // Show author:
    $lcp_display_output .= $this->get_author($post, 'span', 'post-meta-author');

    // Show comments:
    $lcp_display_output .= $this->get_comments($post, 'span', 'post-meta-comments');
  ...
  $lcp_display_output .= '</p>';
    ...
    // Get Posts "More" link:
    $lcp_display_output .= '<div class="post-more-link">';
    $lcp_display_output .= $this->get_posts_morelink($post);
    $lcp_display_output .= '</div>';

  $lcp_display_output .= '</div>'; // close Article Right
